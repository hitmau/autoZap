...
class zap:
    def __init__(self):
        ...
        if bool(self.userpass()[0]):
            userpass = self.userpass()
            self.codusuario = userpass[1]
            cod = userpass[1]
            tel = userpass[2]
            print("tel: " + str(tel))
            self.tel = '+55 ' + str(tel)[:2] + ' ' + str(tel)[2:][:5] + '-' + str(tel)[2:][5:]
            ...
            #-----------------------------------------------------------------------------------------------------
            self.msgEntrada = 'FTBzM.message-in' #'//div[contains(@class,"FTBzM.message-in")]' #_1zGQT._2ugFP.message-in
            self.msgSaida = 'FTBzM.message-out'#'//div[contains(@class,"FTBzM.message-out")]'
            #-----------------------------------------------------------------------------------------------------
    ...
    def userpass(self):
        if bool(mysql.logar('mauriciosist@gmail.com', '123')[0]):
            ...
            #logins = 'mauriciosist@gmail.com', '123'
            if ativo == 'S':
                #if email == logins[0] and senha == logins[1]:
                print("teste de login ok")
                return True, codusuario, telefone
            else:
                return False, codusuario, telefone
                print("teste de login nao ok")
            #break
        else:
    ...
    def getConsultaEntrada(self, inteiro = '0', entrada = '0', saida = '0'):
        self.consultaEntradas = mysql.consultaEntrada(self.codusuario, inteiro, entrada, saida)
    ...
    def zerar(self, string = ''):
        ...
        try:
            ...
            #Remove busca
            try:
                self.driver.find_element_by_class_name('_2heX1').click()
            except:
    ...
    def busca_arvore(self, stringEnt, stringSai):
        """
        Resumo:
        ...
        try:
            ...
            if len(texto) > 0:
                #arq = open(self.parametros, "r")
                #linha = arq.readline()
                for linha in self.getConsultaEntrada('0', stringEnt, stringSai):#self.consultaEntradas:
                    try:
                        textoRespostaMinusculo = linha[7].lower()
                    except:
                        textoRespostaMinusculo = linha[7]
                    print('entranda: banco: ' + str(linha[2]) + ' zap------- ' + str(texto))
                    print('Resposta: banco: ' + str(self.textResposta()[-1][0]) + 's zap------- ' + str(textoRespostaMinusculo))
            ...
            self.retornar()
            print('Erro em busca_arvore ------------------------------------------------------:' + str(es))
    ...
    def buscaTexto(self):
        ...
        try:
            ...
            if self.textPrincipal()[-1][0] != '':
                #print(self.textPrincipal()[-1][1])
                self.busca_arvore(self.textPrincipal()[-1][0], self.textPrincipal()[-1][1])
            else:
